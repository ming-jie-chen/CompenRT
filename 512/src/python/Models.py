...
class CompenRT(nn.Module):
    def __init__(self, gd_net=None, pu_net=None):
        ...
        self.pu_net = copy.deepcopy(pu_net.module) if pu_net is not None else PUNet()
    # simplify trained model to a single sampling grid for faster testing
    def simplify(self, s):
...
if __name__ == '__main__':
    ...
    input3 = torch.randn(4, 3, 512,512).to(device)
    ...
    # sr=Decoder(32,256).to(device)
    # high_level=torch.randn(4,256,64,64).cuda()
    # low_level=torch.randn(4,32,128,128).cuda()
    # x=sr(high_level,low_level,4)
    # print(x.size())
