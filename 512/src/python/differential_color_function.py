...
def euclidean(color1, color2):
    ...
    """
    return torch.sqrt(sum((e1 - e2) ** 2 for e1, e2 in zip(color1, color2))).mean(dim=0)
...
def ciede2000_loss(input: Tensor, target: Tensor,device) -> Tensor:
    Lab_input = rgb2lab_diff(input,device)
    Lab_target=rgb2lab_diff(target, device)
    return ciede2000_diff(Lab_input, Lab_target,device).flatten(1).norm(p=2, dim=1)
